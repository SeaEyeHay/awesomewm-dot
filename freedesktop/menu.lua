...
function menu.build(args)
    ...
    local sub_menu   = args.sub_menu or false
				...
				if args.tag ~= nil then
								bcolor = theme.mytags[args.tag.name].highlight
				end

    local result     = {}
    local _menu      = awful_menu({
								items = before,
								theme = { border_color = bcolor }
				})
            ...
            for _, cat in pairs(result) do
                if cat[1] == v.category then
                    if not menu.has_value(skip_items, v.name) then
																								local function run() awful.spawn(v.cmdline, {tag = args.tag}) end
                        table.insert(cat[2], { v.name, run, v.icon })
                    end
                    break
                end
            end
